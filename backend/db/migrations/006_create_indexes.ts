...
export async function up(client: PgClient): Promise<void> {
  // Create indexes for instrument table
  await client.query(`
    CREATE INDEX IF NOT EXISTS idx_instrument_symbol ON instrument(symbol)
  `);

  await client.query(`
    CREATE INDEX IF NOT EXISTS idx_instrument_active ON instrument(is_active)
  `);
  ...
  await client.query(`
    CREATE INDEX IF NOT EXISTS idx_ohlc_instrument_timeframe ON ohlcv_candle (instrument_id, timeframe, time DESC)
  `);

  // Create indexes for orders table
  await client.query(`
    CREATE INDEX IF NOT EXISTS idx_orders_strategy ON instrument_order(strategy_id)
  `);

  await client.query(`
    CREATE INDEX IF NOT EXISTS idx_orders_current_status ON instrument_order(current_status)
  `);

  await client.query(`
    CREATE INDEX IF NOT EXISTS idx_orders_created_at ON instrument_order(created_at DESC)
  `);
  ...
  await client.query(`
    CREATE INDEX IF NOT EXISTS idx_positions_strategy ON position(strategy_id)
  `);

  await client.query(`
    CREATE INDEX IF NOT EXISTS idx_positions_status ON position(status)
  `);

  // Create indexes for strategies table
  await client.query(`
    CREATE INDEX IF NOT EXISTS idx_strategies_enabled ON strategy(is_enabled)
  `);

  await client.query(`
    CREATE INDEX IF NOT EXISTS idx_strategies_mode ON strategy(execution_mode)
  `);
  ...
  await client.query(`
    ...
    ON position(strategy_id, instrument_id) 
    WHERE status = 'OPEN'
  `);
}
...
export async function down(client: PgClient): Promise<void> {
  ...
  await client.query("DROP INDEX IF EXISTS idx_instrument_active");
  await client.query("DROP INDEX IF EXISTS idx_instrument_symbol");
}
